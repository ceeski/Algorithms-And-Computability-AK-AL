...
1. Open console
2. Write 'python path_to_algorithm.py path_to_input_file' and run exact algorithm, for example if you open console in the main diretory, algorithm is in the file 'algorithm.py' in the 'EXE' diretory and input file is 'file2.txt' in the 'Examples' directory, the command is 'python ./Solution/algorithm.py ./Examples/file2.txt'
3. If you want to run approximate version, add '-a' flag at the end, so for the example above it will be 'python ./Solution/algorithm.py ./Examples/file2.txt -a'
